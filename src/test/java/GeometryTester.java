...
public class GeometryTester {
    ...
    public void testDiagonalDistance() {
        // Same point
        assertEquals(Geometry.findEuclideanDistance(0, 0, 0, 0), 0, Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findEuclideanDistance(5, 0, 5, 0), 0, Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findEuclideanDistance(0, -10, 0, -10), 0, Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findEuclideanDistance(32, -456, 32, -456), 0, Geometry.EQUALITY_THRESHOLD);

        // One equal coordinate
        assertEquals(Geometry.findEuclideanDistance(-1, 0, 0, 0), 1, Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findEuclideanDistance(0, -2, 0, 0), 2, Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findEuclideanDistance(0, 0, -3, 0), 3, Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findEuclideanDistance(0, 0, 0, -4), 4, Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findEuclideanDistance(6, 0, 0, 0), 6, Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findEuclideanDistance(0, 7, 0, 0), 7, Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findEuclideanDistance(0, 0, 8, 0), 8, Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findEuclideanDistance(0, 0, 0, 9), 9, Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findEuclideanDistance(-1, 0, 1, 0), 2, Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findEuclideanDistance(0, 2, 0, -2), 4, Geometry.EQUALITY_THRESHOLD);

        // Distinct x- and y- coordinates
        assertEquals(Geometry.findEuclideanDistance(-5, 7, -3, 6), Math.sqrt(5), Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findEuclideanDistance(490, 234, 436, -50), Math.sqrt(83572), Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findEuclideanDistance(56, 14, 43, 2), Math.sqrt(313), Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findEuclideanDistance(30, -19, 329, 10), Math.sqrt(90242), Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findEuclideanDistance(130, -50, -40, -20), Math.sqrt(29800), Geometry.EQUALITY_THRESHOLD);
    }
    ...
    public void testAngleFrom() {
        ...
        assertEquals(Geometry.findAngleFrom(0, 0, 0, 0), 0, Geometry.EQUALITY_THRESHOLD);
        expectedException.expect(UnsupportedOperationException.class);
        assertEquals(Geometry.findAngleFrom(5, 0, 5, 0), 0, Geometry.EQUALITY_THRESHOLD);
        expectedException.expect(UnsupportedOperationException.class);
        assertEquals(Geometry.findAngleFrom(0, -10, 0, -10), 0, Geometry.EQUALITY_THRESHOLD);
        expectedException.expect(UnsupportedOperationException.class);
        assertEquals(Geometry.findAngleFrom(32, -456, 32, -456), 0, Geometry.EQUALITY_THRESHOLD);

        // One equal coordinate
        assertEquals(Geometry.findAngleFrom(-1, 0, 0, 0), 0, Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findAngleFrom(0, -2, 0, 0), Math.PI / 2, Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findAngleFrom(0, 0, -3, 0), Math.PI, Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findAngleFrom(0, 0, 0, -4), 3 * Math.PI / 2, Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findAngleFrom(6, 0, 0, 0), Math.PI, Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findAngleFrom(0, 7, 0, 0), 3 * Math.PI / 2, Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findAngleFrom(0, 0, 8, 0), 0, Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findAngleFrom(0, 0, 0, 9), Math.PI / 2, Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findAngleFrom(-1, 0, 1, 0), 0, Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findAngleFrom(0, 2, 0, -2), 3 * Math.PI / 2, Geometry.EQUALITY_THRESHOLD);

        // Distinct x- and y- coordinates
        assertEquals(Geometry.findAngleFrom(-5, 7, -3, 6), Math.toRadians(301.60750), Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findAngleFrom(490, 234, 436, -50), Math.toRadians(259.23424), Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findAngleFrom(56, 14, 43, 2), Math.toRadians(222.70938), Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findAngleFrom(30, -19, 329, 10), Math.toRadians(5.5397881), Geometry.EQUALITY_THRESHOLD);
        assertEquals(Geometry.findAngleFrom(130, -50, -40, -20), Math.toRadians(169.99202), Geometry.EQUALITY_THRESHOLD);
    }
    ...
    public void testIntersectBox() {
        ...
        assertEquals(p20.getX(), -5, Geometry.EQUALITY_THRESHOLD);
        assertEquals(p20.getY(), 0, Geometry.EQUALITY_THRESHOLD);
        Point2D p21 = Geometry.intersectBox(0, 0, 80, 0, 50, 0, 30, 67);
        assertEquals(p21.getX(), 80, Geometry.EQUALITY_THRESHOLD);
        assertEquals(p21.getY(), 0, Geometry.EQUALITY_THRESHOLD);
        Point2D p22 = Geometry.intersectBox(-2, -5, 45, -76, -2, -5, 30, 67);
        assertEquals(p22.getX(), -2, Geometry.EQUALITY_THRESHOLD);
        assertEquals(p22.getY(), -5, Geometry.EQUALITY_THRESHOLD);
        Point2D p23 = Geometry.intersectBox(45, -76, -2, -5, -2, -5, 30, 67);
        assertEquals(p23.getX(), -2, Geometry.EQUALITY_THRESHOLD);
        assertEquals(p23.getY(), -5, Geometry.EQUALITY_THRESHOLD);

        // One equal coordinate
        Point2D p24 = Geometry.intersectBox(6, 2, 10, 2, 0, 0, 50, 50);
        assertEquals(p24.getX(), 50, Geometry.EQUALITY_THRESHOLD);
        assertEquals(p24.getY(), 2, Geometry.EQUALITY_THRESHOLD);
        Point2D p25 = Geometry.intersectBox(48, 5, 8, 5, 0, 0, 345, 13);
        assertEquals(p25.getX(), 0 , Geometry.EQUALITY_THRESHOLD);
        assertEquals(p25.getY(), 5, Geometry.EQUALITY_THRESHOLD);
        Point2D p26 = Geometry.intersectBox(3, 7, 3, 79, 0, 0, 296, 305);
        assertEquals(p26.getX(), 0, Geometry.EQUALITY_THRESHOLD);
        assertEquals(p26.getY(), 305, Geometry.EQUALITY_THRESHOLD);
        Point2D p27 = Geometry.intersectBox(47, 432, 47, 45, 0, 0, 479, 345);
        assertEquals(p27.getX(), 47, Geometry.EQUALITY_THRESHOLD);
        assertEquals(p27.getY(), 0, Geometry.EQUALITY_THRESHOLD);

        // Distinct x- and y- coordinates
        Point2D p28 = Geometry.intersectBox(-5, 7, -3, 6, -50, 0, 120, 590);
        assertEquals(p28.getX(), 9, Geometry.EQUALITY_THRESHOLD);
        assertEquals(p28.getY(), 0, Geometry.EQUALITY_THRESHOLD);
        Point2D p29 = Geometry.intersectBox(490, 234, 436, -50, 54, -100, 600, 340);
        assertEquals(p29.getX(), 426.49296, Geometry.EQUALITY_THRESHOLD);
        assertEquals(p29.getY(), -100, Geometry.EQUALITY_THRESHOLD);
        Point2D p30 = Geometry.intersectBox(56, 14, 43, 2, -23, -50, 300, 324);
        assertEquals(p30.getX(), -13.333333, Geometry.EQUALITY_THRESHOLD);
        assertEquals(p30.getY(), -50, Geometry.EQUALITY_THRESHOLD);
        Point2D p31 = Geometry.intersectBox(30, -19, 329, 10, 0, -19, 329, 329);
        assertEquals(p31.getX(), 329, Geometry.EQUALITY_THRESHOLD);
        assertEquals(p31.getY(), 10, Geometry.EQUALITY_THRESHOLD);
